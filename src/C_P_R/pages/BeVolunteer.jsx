...
export default function BeVolunteer() {
  ...
  const handleRequest = async (data) => {
    ...
    if (numberOfVolunteer === 0 || numberOfVolunteer < 0) {
      return Toast.fire({
        icon: "warning",
        title: "Volunteer seats are fulfilled",
      });
    }

    try {
      const response = await axios.post(
        `${import.meta.env.VITE_API_KEY}/becomeVolunteer?id=${_id}`,
        doc
      );
      if (response.data.message === "Yes includes") {
        return Toast.fire({
          icon: "error",
          title: "You have already volunteer",
        });
      }
  ...
  return (
    <>
      <Helmet>
        <title>Volunify | be volunteer</title>
        <meta name="description" content="This is a be volunteer route." />
      </Helmet>
      <form onSubmit={handleSubmit(handleRequest)}>
        <div className="w-60 mx-auto my-5">
          <img
            src={photo}
            className="object-cover object-center w-full rounded-md h-72 bg-gray-500 dark:bg-gray-500 pointer-events-none"
            alt=""
          />
        </div>
        <div className="lg:w-3/4 m-auto p-5 grid grid-cols-1 lg:grid-cols-3 border-y-2 border-dashed">
          ...
          <div className="space-y-4 lg:border-l-2 border-dashed px-2 lg:col-span-2">
            ...
            <input
              ...
              id=""
              readOnly
              defaultValue={`${numberOfVolunteer} (seat)`}
              className="input input-bordered w-full"
            />
            <input
              type="text"
        ...
        <div className="lg:w-3/4 m-auto p-5 grid grid-cols-1 lg:grid-cols-3 border-y-2 border-dashed">
          ...
          <div className="space-y-4 lg:border-l-2 border-dashed px-2 lg:col-span-2">
            <input
              ...
              id=""
              readOnly
              defaultValue={`${organizationName} (name)`}
              className="input input-bordered w-full"
            />
            <input
              type="text"
              ...
              id=""
              readOnly
              defaultValue={`${organizationEmail} (email)`}
              className="input input-bordered w-full"
            />
          </div>
        </div>
        <div className="lg:w-3/4 m-auto p-5 grid grid-cols-1 lg:grid-cols-3 border-t-2 border-dashed">
          ...
          <div className="space-y-4 lg:border-l-2 border-dashed px-2 lg:col-span-2">
            <input
              ...
              className="input input-bordered w-full"
              {...register("name", {
                required: user?.displayName ? false : "Please enter your name",
              })}
            />
        ...
        <input
          type="submit"
          value="Request"
          className="btn btn-block lg:w-1/2 block mx-auto"
        />
      </form>
    </>
  );
